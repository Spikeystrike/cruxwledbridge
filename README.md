...
3) Input your grid size, then select whether the grid is standard or alternating. In alternating mode, `C` is the total number of possible horizontal positions and may be even or odd. Choose whether the top row starts with the 5 cm or 15 cm pattern. For example, with `C=22`, each row contains 11 LEDs; with an odd `C`, alternating rows differ by one LED while their snake IDs remain contiguous. Tap the grid corners in the order top left, top right, bottom right, bottom left and press send. You can then click individual grid positions to disable or re-enable them and save the adjusted mapping.
