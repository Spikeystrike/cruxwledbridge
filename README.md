...
3) Configure `wled_controllers` in `config/config.py`. Each controller has an inclusive range of global physical LED IDs. The example uses the documentation-only address `192.0.2.10` for IDs `0-399`; replace it and the range with your actual WLED setup. The bridge translates these IDs to zero-based controller-local IDs and sends colors through WLED's `/json/state` API. The physical LED IDs themselves are assigned in Wall Creation, so no separate `hole2LEDS` mapping is needed.
2) open `https://<bridge>/cruxwledbridge/listwalls?gym=<gymslug>` and select the wall you want to map
3) Input your grid size, then select whether the grid is standard or alternating. In alternating mode, `C` is the total number of possible horizontal positions and may be even or odd. Choose whether the top row is indented or not indented. For example, with `C=22`, each row contains 11 LEDs; with an odd `C`, alternating rows differ by one LED. Select the physical location of LED 0 (top left, top right, bottom left, or bottom right) and whether the cable snakes horizontally through rows or vertically through columns. The default is LED 0 at the bottom left with a vertical cable run. Tap the grid corners in the order top left, top right, bottom right, bottom left and press send. You can then click individual grid positions to disable or re-enable them and save the adjusted mapping; active physical LED IDs stay contiguous in the selected cable order and are stored directly for the Crux holds.
