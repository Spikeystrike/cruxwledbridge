...
3) Input your grid size, then select whether the grid is standard or alternating. In alternating mode, `C` is the total number of possible horizontal positions and may be even or odd. Choose whether the top row is indented or not indented. For example, with `C=22`, each row contains 11 LEDs; with an odd `C`, alternating rows differ by one LED. Select the location of logical hole 0 (top left, top right, bottom left, or bottom right) and whether the cable snakes horizontally through rows or vertically through columns. The default is hole 0 at the bottom left with a vertical cable run. Tap the grid corners in the order top left, top right, bottom right, bottom left and press send. You can then click individual grid positions to disable or re-enable them and save the adjusted mapping. Active logical hole IDs stay contiguous in the selected cable order; `hole2LEDS` performs the separate mapping from those hole IDs to the actual physical LEDs.
